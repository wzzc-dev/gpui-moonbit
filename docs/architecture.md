...
OP_IMAGE            u8 | max_w i32 | max_h i32 | fit i32 | len u32 | utf8[len]
                                                  (画像 leaf; issue #103)
```
...
`OP_IMAGE`（issue #103）は `UiNode::Image` leaf を push する。`max_w`/`max_h` は**上界**（`<= 0` = 無制約）で、固定画框ではない — 画像の内在サイズは commit 時には未知なので、固定枠を強制すると呼び出し側が縦横比を推測することになり、外した画像はすべて違う形の枠にレターボックスされる。代わりに画像は自身のサイズでレイアウトされ、`Img::request_layout` が復号後の内在サイズから制約のない軸を導出する。`fit` は `[image_fit]` セクションの `IMAGE_FIT_*` id で、enum 外の値は `GPUI_STATUS_INVALID_IMAGE_FIT`（`-17`）で**バッファごと**拒否する（gpui の `ObjectFit` に「不明」を表す variant が無く、寛容にデコードすると呼び出し側がオペランドを落とされたことに気付けない）。`src` は `http(s)://` URL・`data:` URI・ファイルパスのいずれかで、解決は**描画時**に gpui のアセットパイプラインが行う — リモート画像が commit をブロックすることはない。`http(s)://` は `App::http_client()` 経由でダウンロードされ、その既定値は `NullHttpClient`（全リクエストが失敗する）なので `run_window` が `ReqwestHttpClient`（`gpui-sys/src/http.rs`、`zed-reqwest` + 専用の Tokio current-thread ランタイム）を差し込む。`data:` だけは gpui にローダーが無いため `image_source_from_data_uri` が base64 復号とフォーマット判定を行う。読み込み中と失敗時は gpui の `with_loading`/`with_fallback` が `max_w`×`max_h` のプレースホルダカードに差し替える（遅延は gpui 固有の `LOADING_DELAY` = 200 ms）。
